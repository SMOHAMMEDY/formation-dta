...
public static Pizza getPizza(int id,String code, String nom, double prix){
	...
	public static void main(String[] args) {
		...
		Pizza[] listePizzas = { getPizza(0,"PEP","Pépéronie",12.50),
								getPizza(1,"MAR","Margarita",14.50),
								getPizza(2,"REI","La Reine",11.50),
								getPizza(3,"FRO","La 4 Fromages",12.00),
								getPizza(4,"CAN","La Cannibale",12.50),
								getPizza(5,"SAV","La Savoyarde",13.00),
								getPizza(6,"ORI","L'Oriantale",13.50),
								getPizza(7,"IND","L'Indienne",14.00)};
	 ...
	 // commencer les conditions selon ce qu'on a selectionner comme menu
	  ...
	  // le menu 1
		...
		for (int i = 0; i < listePizzas.length; i++) {
	            ...
	            System.out.println(listePizzas[i].code + " -> " + listePizzas[i].nom + "(" + listePizzas[i].prix + ")" ); 
			
		}
		System.out.println("-------" + listePizzas[0].nbPizzas + " " + "pizzas créées depuis l'initialisation du programme");
		...
		Pizza[] newTab = Arrays.copyOf(listePizzas, listePizzas.length+1); 
			...
			// Ajout de la nouvelle pizza à l'index taille listePizzas ( on fais appel à la fonction get pizza)
		newTab[listePizzas.length] = getPizza (newTab[listePizzas.length-1].id+1,code, nomPizza, prix); // [listePizza.lenth]= i = le nombre de ligne de liste pizza qui a l'origine été 8
		...
		for (int i = 0; i < listePizzas.length; i++) {
	            ...
	            System.out.println(listePizzas[i].id + " " + listePizzas[i].code + " - " + listePizzas[i].nom + "(" + listePizzas[i].prix + ")" ); 
	   }         
		...
		listePizzas[index].code = code;
		listePizzas[index].nom = nomPizza;
		listePizzas[index].prix = prix;
				
		// afficher le tableau liste_pizzas ( pour line par line avec esthetique )
		for (int i = 0; i < listePizzas.length; i++) {
		...
		// afficher le tableau liste_pizzas ( pour line par line avec estetique )
				...
				// parcourir le l'ancien tableau pour remplir le nouveau à l'exception de la pizza supprimée
